---
title: "Impact of Storms to US Population and Ecomony"
author: "Martí Cuquet"
output: 
  html_document:
    keep_md: true
...
This means we have `r num.ignored.obs` ignored observations, ie around
`r round(100 * num.ignored.obs / nrow(stormdf))`% of the total.
...
After this data processing, only `r nrow(other)` observations are ignored (ie
labelled as "other"), which makes a
`r round(100 * nrow(other) / nrow(stormdf), digits = 4)`% of the total.
...
```{r fig.cap = "Fatalities and injuries caused by the six most harmful US storm events types. The effect of the remaining event types are included in \"other\".", fig.width=10}
print(g1)
```
...
```{r fig.cap = "Economic damage caused by the six most harmful US storm events types. The effect of the remaining event types are included in \"other\".", fig.width=10}
print(g2)
```
